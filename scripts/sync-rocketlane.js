...
const PROJECT_FIELD_LABELS = {
  ...
  // REVERTED to Project-sourced 28 Aug 2026, alongside internalStatusTag
  // above. These eight were each individually confirmed live earlier this
  // session as genuine, populated PROJECT fields — moved to Company per
  // Amy's broad correction, then confirmed BROKEN there in two concrete
  // cases (Internal Accounting Status, Accounting Go Live Date): the guessed
  // Company-side label resolves with no warning but returns null for real
  // clients. Reverting all eight together rather than waiting to find each
  // one broken individually — 2 for 2 is enough of a pattern. If any of
  // these turn out to genuinely need a Company source after all, they should
  // move back with a confirmed fieldId in hand, not a same-name guess.
  ...
  streetGoLiveDateActual: 'Actual Go Live Date / Out of Set Up Mode', // still tentative as a label match regardless of object — see history below
...
// were confirmed populated on real clients early on. onboardingSpecialist,
// accountingStatus, and the four restart fields below were never confirmed
// as living on either object — Project or Company — so they stay here per
// Amy's original instruction, as the only guess available; check the first
// run's "Missing field mapping" warnings against this list, and if they
// resolve but show suspiciously blank/wrong values the way
// internalStatusTag and accountingGoLiveDateActual did, that's the same
// failure mode and they likely belong back on Project too.
...
async function buildCompanyRecord(company, projects, projectFieldIds, companyFieldIds, userIndex) {
  ...
  const agentLiveWithStreet = onboarding ? getBooleanField(onboarding, projectFieldIds.agentLiveWithStreet) : false;
  const streetGoLiveDate = onboarding ? getDateField(onboarding, projectFieldIds.streetGoLiveDateActual) : null;
  const streetUsage = onboarding ? getMultiChoiceField(onboarding, projectFieldIds.streetUsage) : null;
  ...
  return {
    ...
    clientAccountingEnabled: onboarding ? Boolean(getBooleanField(onboarding, projectFieldIds.clientAccountingEnabled)) : false,
    streetPayments: {
