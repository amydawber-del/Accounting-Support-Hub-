...
const COMPANY_FIELD_LABELS = {
  ...
  clientAccountingEnabled: 'Client Accounting Enabled',
  // MOVED to Company-sourced 28 Aug 2026 per Amy — all clients were showing
  // No/blank despite ~20 real Street Payments clients existing, reading from
  // Project. Labels below are a first guess (matching Amy's own phrasing,
  // the same approach that worked for Agent Live with Street and Accounting
  // Go Live Date) — NOT yet confirmed. Given the Street Network ID lesson,
  // if these still show blank after this change, check for a trailing colon
  // in the real label before assuming the wording itself is wrong.
  streetPaymentsCustomer: 'Street Payments Client',
  streetPaymentsVerificationStatus: 'Verification Status',
