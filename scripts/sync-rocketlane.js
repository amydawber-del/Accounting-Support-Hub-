...
// grouping, and confirmed that real clients do carry data here. Labels below
// are unverified against a real fieldId — the MCP tool connected to this chat
// refused objectType=COMPANY lookups (only PROJECT/TASK/USER allowed), even
// though Rocketlane's real REST API supports it. Check the first run's
// "Missing field mapping" warnings against this list.
//
// MOVED HERE 28 Aug 2026, per Amy's explicit correction: everything below
// from accountingOwner down was previously mapped and read from the
// Accounting Onboarding PROJECT — each one was individually confirmed to
// exist there via live field lookups. Amy has since clarified that the real
// source of truth for all of these is the Company/Account record instead.
// Since a Rocketlane field is bound to one object type, this means either (a)
// a same- or similar-named field exists separately on Company, or (b) the
// Project-side field I found isn't the one actually in use. Either way, these
// now use the identical label text as a first guess, since that's the exact
// wording confirmed to exist somewhere in this tenant. If the first sync run
// after this change logs "Missing field mapping" for any of these, the real
// Company-side label differs and needs Amy to confirm the exact wording —
// don't assume it's just a typo in this guess.
const COMPANY_FIELD_LABELS = {
  ...
  // NEW — never previously found on the Project either; no lead on the real
  // label at all yet. Using the plain name Amy gave as the only guess available.
  onboardingSpecialist: 'Onboarding Specialist',
  accountingOwner: 'Accounting Owner',
  // NEW — no candidate field found anywhere before (distinct from Internal
  // Accounting Status). Using Amy's plain name as the only guess available.
  ...
  agentLiveWithStreet: 'Agent Live on Street (Accounting Tracker)',
  // Still tentative regardless of which object it's on — see the long note
  // this field carried before the move; its label doesn't literally say
  // "Street", so it needs an eyeball check against a known go-live date even
  // once the field mapping itself resolves cleanly.
  streetGoLiveDateActual: 'Actual Go Live Date / Out of Set Up Mode',
...
async function buildCompanyRecord(company, projects, projectFieldIds, companyFieldIds, userIndex) {
  ...
  // MOVED to Company-sourced reads 28 Aug 2026 — see the long note on
  // COMPANY_FIELD_LABELS above for why. redFlag and segment are deliberately
  // NOT in this list — redFlag wasn't part of Amy's correction, and segment
  // was explicitly confirmed to stay on the Project.
  const internalStatusTag = getChoiceField(company, companyFieldIds.internalStatusTag);
  const redFlag = onboarding ? getBooleanField(onboarding, projectFieldIds.redFlag) : false;
  const agentLiveWithAccounting = getBooleanField(company, companyFieldIds.agentLiveWithAccounting);
  ...
  const streetGoLiveDate = getDateField(company, companyFieldIds.streetGoLiveDateActual);
  ...
  return {
    ...
    accountingOwner: getSingleUserField(company, companyFieldIds.accountingOwner, userIndex),
    // NEW — no confirmed field found anywhere for this before; see the
    // COMPANY_FIELD_LABELS note. Using getTextField as a generic, safe
    // default since the real field's type isn't known — if it's actually a
    // Single User field, this will visibly show a raw number instead of a
    // name, which is an easy tell that it needs the same SINGLE_USER
    // treatment as Accounting Owner once confirmed.
    onboardingSpecialist: getTextField(company, companyFieldIds.onboardingSpecialist),
    // NEW — distinct from Internal Accounting Status (internalStatusTag
    // below). No confirmed field found before; type assumed SINGLE_CHOICE
    // since every other "...Status" field in this tenant has been that type.
    accountingStatus: getChoiceField(company, companyFieldIds.accountingStatus),
    clientAccountingEnabled: getBooleanField(company, companyFieldIds.clientAccountingEnabled),
    streetPayments: {
      customer: getBooleanField(company, companyFieldIds.streetPaymentsCustomer),
      verificationStatus: getChoiceField(company, companyFieldIds.streetPaymentsVerificationStatus),
    },
    ...
    // Kept on the Project per Amy's explicit confirmation (28 Aug 2026) — the
    // one field in this correction that was already verified working there.
...
async function main() {
  ...
  // Company is included if it has ANY recognised Accounting-related project —
  // onboarding, reconciliation, or training. The previous version of this loop
  // required an onboarding-classified project specifically
  // (`!linkedProjects.onboarding`), which silently dropped any company whose
  // only linked project was Reconciliation or Training (e.g. a long-settled
  // client with just a training record, or a client mid-reconciliation-only
  // reset with no live onboarding project). buildCompanyRecord already handles
  // a missing `onboarding` project gracefully everywhere (`onboarding ? ... :
  // null`), so this was purely an overly-strict gate, not a data limitation.
  const skippedNoAccountingProject = [];
  for (const company of companies) {
    const linkedProjects = projectsByCompany.get(company.companyId);
    const hasAnyAccountingProject =
      linkedProjects && (linkedProjects.onboarding || linkedProjects.reconciliation || linkedProjects.training);
    if (!hasAnyAccountingProject) {
      skippedNoAccountingProject.push(company.companyName);
      continue;
    }
  ...
  console.log(`[sync] Companies skipped (no onboarding/reconciliation/training project found): ${skippedNoAccountingProject.length}`);
  if (skippedNoAccountingProject.length) {
    console.log('[sync]   Sample of skipped company names (first 20) — cross-check a few of these against Rocketlane directly if the count looks too high:');
